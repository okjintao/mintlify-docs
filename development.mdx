...
Follow these steps to install and run Berachain on your operating system.

<Steps>
<Step title="Install the Berachain CLI">
...
By default, Berachain uses port 3000. You can customize the port Berachain runs on by using the `--port` flag. For example, to run Berachain on port 3333, use this command:

```bash
mint dev --port 3333
```

If you attempt to run Berachain on a port that's already in use, it will use the next available port:
...
Please note that each CLI release is associated with a specific version of Berachain. If your local preview does not align with the production version, please update the CLI:
