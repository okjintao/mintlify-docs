...
description: Learn about Berachain's Proof-of-Liquidity mechanism and how it works
---
...
Berachain's Active Set of validators (validators participating in consensus) is determined by validators' `$BERA` stake, with a minimum of `10,000 $BERA` and a maximum cap of `500,000 $BERA`. The top 100 validators ranked by stake are in the Active Set. Within the Active Set, a validator's probability of proposing a block is proportional to their staked `$BERA` — more `$BERA` staked increases the likelihood of proposing a block.
...
Learn about [integrating with Berachain's incentive system](/berachain/learn/pol/incentives) and [Reward Vault design](/berachain/learn/pol/rewardvaults).
