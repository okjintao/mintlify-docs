---
title: "$BGT Token"
description: "What Is the BGT Token and how it works"
---
...
Users can see available earning options at [Berachain Hub Pools](https://hub.berachain.com/pools).
...
`$BGT` is used to vote on governance proposals. `$BGT` holders are responsible for a wide variety of ecosystem decisions (see [Governance](/general/governance/overview)).
...
Users who are boosting validators with their `$BGT` earn a share of Berachain core dApp fees, namely fees from BEX and HoneySwap. This is done via the `FeeCollector` contract.
