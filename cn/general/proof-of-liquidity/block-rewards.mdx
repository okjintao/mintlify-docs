...
description: "How block rewards and BGT emissions work in Berachain's Proof-of-Liquidity"
---
...
![chart showing how emissions scale with BGT delegation](/images/emission-scale-bgt-delegation-chart.png)
...
`$BGT` is emitted to reward vaults on a per-block basis via the Distributor. Distributions for a given block are performed during processing for the following block. Distribution creates `$BGT` that is then claimable by Reward Vault stakers.
...
![Complex Example](/images/complex-emission.png)
...
![Boost APR Example](/images/boost-apr-example.png)
